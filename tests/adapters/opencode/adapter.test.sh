...
SKILLEOF
  ...
  [[ -f "$dst/.opencode/skills/foo/references/guide.md" ]] || { echo 'reference missing'; result=1; }
  [[ -f "$dst/.opencode/skills/foo/assets/template.md" ]] || { echo 'asset missing'; result=1; }
  [[ -f "$dst/.opencode/skills/foo/scripts/run.sh" ]] || { echo 'script missing'; result=1; }
  [[ -f "$dst/.opencode/skills/foo/agents/openai.yaml" ]] || { echo 'openai.yaml missing'; result=1; }
  grep -RIl '.platform/\|DISPATCHER.md' "$dst/.opencode/skills/foo" | grep -q . \
    && { echo 'neutral path leaked from OpenCode bundle'; result=1; }
  rm -rf "$src" "$dst"
  return $result
}

test_oc_translate_skills_propagates_copy_bundle_failure() {
  local src; src="$(mktemp -d)"
  local dst; dst="$(mktemp -d)"
  ...
  rm -rf "$src" "$dst"
  [[ $adapter_status -ne 0 ]] || { echo 'OpenCode adapter masked copy_skill_bundle failure'; return 1; }
}

test_oc_translate_skills_propagates_bundle_rewrite_failure() {
  local src; src="$(mktemp -d)"
  local dst; dst="$(mktemp -d)"
  mkdir -p "$src/skills/a" "$src/skills/b"
  touch "$src/skills/a/SKILL.md" "$src/skills/b/SKILL.md"
  ...
  eval "$original_rewrite"
  rm -rf "$src" "$dst"
  [[ $adapter_status -ne 0 ]] || { echo 'OpenCode adapter masked bundle rewrite failure'; return 1; }
}
